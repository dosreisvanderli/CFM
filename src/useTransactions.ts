...
const syncDone = useRef(false);
  ...
  const handleSaveTransaction = async (formData: any, editingId: string | null, currentMonth: number, currentYear: number) => {
    ...
    try {
      if (editingId) {
        ...
        // Robust parsing of installments
        const numInstallments = Math.max(1, parseInt(formData.installments) || 1);
        const startInstallment = Math.max(1, parseInt(formData.currentInstallment) || 1);
        
        // Calculate correctly: number of installments to create
        // If user says "Starting at 1 of 5 total", we create 5.
        // If user says "Starting at 3 of 5 total", we create 3, 4, 5. (3 items).
        let totalToGenerate = 1;
        if (numInstallments > 1 || startInstallment > 1) {
          totalToGenerate = Math.max(1, numInstallments - startInstallment + 1);
        }
        
        // Handle numeric format robustly (comma vs dot)
        let valString = formData.val.toString().trim();
        // Remove thousands separator if any (like 1.000,00)
        ...
        const baseAmount = (parseFloat(valString) || 0).toFixed(2);
        const parsedDay = Math.max(1, Math.min(31, parseInt(formData.dueDay) || new Date().getDate()));
        
        // Use provided month/year or fallback to current view month/year
        let baseMonth = currentMonth;
        let baseYear = currentYear;
        if (formData.dueMonth) baseMonth = parseInt(formData.dueMonth) - 1;
        if (formData.dueYear) baseYear = parseInt(formData.dueYear);

        const newGroupId = (numInstallments > 1 || startInstallment > 1) ? generateId() : null;
        ...
        for (let i = 0; i < totalToGenerate; i++) {
          ...
          m = ((m % 12) + 12) % 12; // Handle negative if any, but m should be > 0 here
          ...
          // Installment 1 can be PAGO, others are usually PENDENTE if in future
  ...
  const handleCopyTransactions = async (ids: string[], customDate?: { day: number, month: number, year: number }) => {
    if (!user || ids.length === 0) return;
    try {
      ...
      if (user.isOfflineMode) {
        ...
        itemsToCopy.forEach(item => {
          let day = customDate ? customDate.day : item.day;
          let month = customDate ? customDate.month : item.month + 1;
          let year = customDate ? customDate.year : item.year;
          
          if (!customDate && month > 11) { 
            month = 0; 
            year++; 
          }
